...
class Base(commands.Cog):
    ...
    @app_commands.command(name="pleaseexplain", description="what's goodie")
    async def pleaseexplain(self, interaction: discord.Interaction, term: str):
        ...
        create_dictionary_image(term, definition)
        filename = "output.png"
        url = f'attachment://{filename}'
        file = discord.File("lib/data/image_data/image_cache/output.png", filename=filename)
        embed = create_def_embed(url, creator, requestor, created_at, updated_at)
        await interaction.response.send_message(embed=embed, file=file)

    @app_commands.command(name="define", description="add a custom definition to your server dictionary")
    ...
    async def dictionary(self, interaction: discord.Interaction, page: int = 1):
        ...
        for value in values[start_index:end_index]:
            target_values.append(value[0])
            # cache id : user object pairs in dictionary to prevent duplicate fetching
            if value[1] not in creator_cache.keys():
                creator = interaction.client.get_user(value[1])
                if creator is None:
                    creator = await interaction.client.fetch_user(value[1])
                creator_cache[value[1]] = creator
            else:
                creator = creator_cache[value[1]]
